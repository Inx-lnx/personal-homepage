...
(function initLightbox(){const imgs=[...document.querySelectorAll(".petal img")];if(!imgs.length)return;let index=0;const overlay=document.createElement("div"),image=document.createElement("img"),close=document.createElement("button"),prev=document.createElement("button"),next=document.createElement("button"),counter=document.createElement("span");overlay.className="lightbox";close.className="lb-close";close.textContent="×";close.setAttribute("aria-label","关闭");prev.className="lb-prev";prev.textContent="‹";prev.setAttribute("aria-label","上一张");next.className="lb-next";next.textContent="›";next.setAttribute("aria-label","下一张");counter.className="lb-count";overlay.append(image,close,prev,next,counter);document.body.appendChild(overlay);const render=()=>{const img=imgs[index],full=img.dataset.full||img.src;image.src=img.src;image.alt=img.alt;image.dataset.full=full;const big=new Image();big.onload=()=>{if(image.dataset.full===full)image.src=full};big.src=full;counter.textContent=(index+1)+" / "+imgs.length};const show=i=>{index=(i+imgs.length)%imgs.length;render()};const hide=()=>overlay.classList.remove("is-open");imgs.forEach((img,i)=>img.addEventListener("click",()=>{show(i);overlay.classList.add("is-open")}));prev.addEventListener("click",e=>{e.stopPropagation();show(index-1)});next.addEventListener("click",e=>{e.stopPropagation();show(index+1)});close.addEventListener("click",hide);overlay.addEventListener("click",e=>{if(e.target===overlay)hide()});document.addEventListener("keydown",e=>{if(!overlay.classList.contains("is-open"))return;if(e.key==="Escape")hide();if(e.key==="ArrowLeft")show(index-1);if(e.key==="ArrowRight")show(index+1)})})();
